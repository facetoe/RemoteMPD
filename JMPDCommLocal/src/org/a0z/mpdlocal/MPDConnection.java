...
import org.a0z.mpdlocal.exception.MPDConnectionException;
import org.a0z.mpdlocal.exception.MPDNoResponseException;
...
public abstract class MPDConnection {
    ...
    protected static final String POOL_THREAD_NAME_PREFIX = "pool";

    protected InetAddress hostAddress;
    protected int hostPort;

    protected int[] mpdVersion;
    protected List<MPDCommand> commandQueue;
    ...
    protected final static int MAX_CONNECT_RETRY = 3;
    protected final static int MAX_REQUEST_RETRY = 3;
    ...
    protected int[] connect() throws MPDServerException {
    ...
    protected int[] innerConnect() throws MPDServerException {
    ...
    private List<String> innerSyncedWriteAsyncRead(MPDCommand command)
        ...
        while (!dataReaded) {
            try {
                ...
                System.err.println("Socket timeout while reading server response : " + e);
            } catch (IOException e) {
                throw new MPDConnectionException(e);
            }
        }
    ...
    class MpdCallable extends MPDCommand implements Callable<MPDCommandResult> {
        ...
        public MPDCommandResult call() throws Exception {
            ...
            if (result.getResult() == null) {
                ...
                System.err.println("MPD command " + command + " failed after " + retry + " attempts : " + result.getLastexception().getMessage());
            }
            return result;
        }
    }
            ...
            public void run() {
                for (int i = 0; i < maxThreads; i++) {
                    try {
                        ...
                        System.err.println("All connection refresh failure : " + e);
                    }
                }
            }
        }).start();
