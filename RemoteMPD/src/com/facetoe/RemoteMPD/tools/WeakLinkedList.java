...
package com.facetoe.RemoteMPD.tools;
...
/**
 * A List implementation that uses WeakReferences and is backed by a
 * custom linked list implementation. This list tries to gracefully handle
 * elements dropping out of the list due to being unreachable. <br>
 * <br>
 * An Iterator or ListIterator on this list will not fail if an element disapears
 * due to being unreachable. Also while iterating no reachable element will ever
 * be skipped.
 * 
...
public class WeakLinkedList<T> implements List<T> {
	...
	/**
	 * Returns an Iterator that gracefully handles expired elements. The Iterator
	 * cannot ensure that after calling hasNext() successfully a call to next() will
	 * not throw a NoSuchElementException due to element expiration due to weak references. <br>
		...
		synchronized (LOCK) {
			...
			while ((deadNode = (WeakListNode) queue.poll()) != null) {
				...
				if (!deadNode.isRemoved()) {
					if (listName != null) {
						Log.e("WeakLinkedList", "Error : " + listName
	...
	private class DurableListIterator implements ListIterator<T> {
		...
		/**
		 * Inspects the previous and next nodes to see if either have been removed from
		 * the list because of a removed reference
		 */
